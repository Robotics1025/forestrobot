...
# Shared file used to hand the live fire coordinates to the drone controllers.
# Both controllers resolve to the same project root, so the path is shared.
...
class Fire(Supervisor):
    ...
    def publish_fire_targets(self):
        """Write the world (x, y) coordinates of every currently burning tree to
        the shared file so the drones can navigate straight to the fire."""
        targets = []
        for tree in self.trees:
            if tree.fire:
                targets.append([tree.translation[0], tree.translation[1]])
        try:
            with open(FIRE_TARGETS_FILE, 'w') as f:
                json.dump(targets, f)
        except OSError:
            pass
    ...
    def run(self):
        ...
        self.publish_fire_targets()  # start with whatever is currently alight
        while True:
            ...
            if start_fire_now:
                ...
                # Keep the drones informed of where the fire currently is.
                self.publish_fire_targets()
            else:
