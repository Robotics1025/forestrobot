...
class Mavic (Robot):
    ...
    def read_fire_targets(self):
        """Read the live fire coordinates published by the fire supervisor.
        Returns:
            list of [x, y] world coordinates of currently burning trees (may be empty).
        """
        try:
            with open(FIRE_TARGETS_FILE, 'r') as f:
                targets = json.load(f)
            ...
            if altitude > target_altitude - 1:
                ...
                if self.getTime() - t1 > 0.1:
                    if self.img_coord_fire:
                        ...
                        fire_targets = self.read_fire_targets()
                        if fire_targets:
                            # Navigate straight to the real fire coordinates.
                            yaw_disturbance, pitch_disturbance = self.go_to_fire(
                                fire_targets)
                        else:
                            # No active fire known: patrol the forest.
